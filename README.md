...
- [EKS cluster with EBS and EFS and ALB](#eks-cluster-with-ebs-and-efs-and-alb)
  ...
  - [Add EFS to the Cluster](#add-efs-to-the-cluster)
    ...
    - [Create an IAM policy and role](#create-an-iam-policy-and-role-1)
...
This process will generate a new kubeconfig file that can be handed to another user.

The user will need then to export the KUBECONFIG variable to point at this file.
...
Now share the file in $KUBECONFIG with the user. Keep in mind that this file contains an administrative bearer token to the cluster, so handle it with the same care as you handle the sharing of any other credential or password.
...
The cluster needs an OIDC provider. Note that when the cluster is created with the `--with-oidc`, then this block below is redundant.
